...
class Master(commands.Cog):
    ...
    @commands.command(name='checkreqjr', aliases=['checkreqsjr'])
    @commands.cooldown(1, 0)
    async def checkreqjr(self, ctx: commands.Context, ign: str, cute_name: str = None):
        await ctx.trigger_typing()
        ...
        embed.add_field(name="Your Stats", value=f"Slayer Req: 400000 xp | "
                                                 ...
                                                 f"Cata req: level 25 | "
                                                 f"Your Cata: **{dungeon_lvl}** | {p if dungeon_req else np} \n"
                                                 f"Weight req: 2100 senither weight | "
                                                 f"Your Weight: {weight} | {p if weight_req else np}", inline=False)
        embed.set_footer(text=f'{ign} | {selected_profile["cute_name"]}')
