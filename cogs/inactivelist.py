...
class InactiveList(commands.Cog):
    ...
    async def inactiveadd(self, ctx, ign : str, afktime = None):
        if afktime == None:
            embed = discord.Embed(title=f'Error', description='No time inputted \nEnter time in days Ex: 10d for 10 days',
                                colour=0xFF0000)
        ...
        conn.commit()
        conn.close()
        embed = discord.Embed(title=f'Success', description=f'{request["name"]} has been added to Inactive list until {datetime.datetime.fromtimestamp(afktime).strftime("%A, %B %d, %Y %I:%M:%S")}',
